...
with sql.connect("app.db") as con:
    ...
    with open(airline_csv) as file_handler:
        ...
        for c in csv:
            cur.execute("INSERT INTO airline (iata, name) VALUES (?,?)",(c[1], c[0]))
    ...
    with open(airports_csv) as file_handler:
        ...
        headers = csv.pop(0)
        for c in csv:
            cur.execute("INSERT INTO airport (iata, name, city, country, latitude, longitude, time_zone, dst) VALUES (?,?,?,?,?,?,?,?)",(c[1], c[2], c[3], c[4], c[5], c[6], c[8], c[9]))
