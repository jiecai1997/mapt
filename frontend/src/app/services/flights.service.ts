...
export class FlightsService {
  ...
  // anything to change here? Similar to add right?
  modifyTrip(uid:number, sessionToken:number, tripName:string, flights:Flight[]){
    const reqBody = {'userID': uid, sessionToken: sessionToken, 'tripName': tripName, 'flights': flights};
    console.log('reqBody', reqBody);

    const httpOptions = this.createAuthOptions();

    return this.http.put(this.serverURL + '/trips/modify', reqBody, httpOptions).subscribe(result => {
      if(result['success'] == 'true'){
        return 'Success';
      }
      else{
  ...
  public getStats(uid:number){
    ...
    return this.http.get(this.serverURL + '/stats/?uid', httpOptions);
  }
