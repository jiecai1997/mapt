...
export class FlightlistComponent implements OnInit {
    ...
    if(this.isAddTrip){
      ...
      this.flightsService.getTrips().subscribe(
        tripsObj => {
            ...
            this.airlineSuggestions = this.trips.map(trip => trip.flights.map(flight => []));
            this.airportSuggestions = this.trips.map(trip => trip.flights.map(flight => {
              return {dep: [], arr: []}
            }));
            this.errors = this.trips.map(trip => '');
  ...
  addFlight(tripIndex: number): void{
    ...
    this.airlineSuggestions[tripIndex].push([]);
    this.airportSuggestions[tripIndex].push({arr: [], dep: []})
  }
  ...
  getAirlineSuggestions(tripIndex: number, flightIndex: number, search: string): any {
    this.flightsService.autocompleteAirlines(search).subscribe(result => {
      this.airlineSuggestions[tripIndex][flightIndex] = result['airlines'];
    });
  }

  getAirportSuggestions(tripIndex: number, flightIndex: number, isDep: boolean, search: string): any {
    this.flightsService.autocompleteAirports(search).subscribe(result => {
      this.airportSuggestions[tripIndex][flightIndex][isDep ? 'dep' : 'arr'] = result['airports'];
    });
  }
  ...
  removeFlight(tripIndex: number, flightIndex: number): void{
    ...
    this.airlineSuggestions[tripIndex].splice(flightIndex, 1);
    this.airportSuggestions[tripIndex].splice(flightIndex, 1);
  }
