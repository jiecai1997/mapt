...
export class FlightlistComponent implements OnInit {
  ...
  getAirlineSuggestions(search: string): any {
    if(search == undefined || search == ''){
      return [];
    }else if(this.airlineSuggestions[search] != undefined){
      return this.airlineSuggestions[search];
    }else{
      this.airlineSuggestions[search] = [];

      this.flightsService.autocompleteAirlines(search).subscribe(result => {
        this.airlineSuggestions[search] = result['airlines'];
      });
    }
  }

  getAirportSuggestions(search: string): any {
    if(search == undefined || search == ''){
      return [];
    }else if(this.airportSuggestions[search] != undefined){
      return this.airportSuggestions[search];
    }else{
      this.airportSuggestions[search] = [];

      this.flightsService.autocompleteAirports(search).subscribe(result => {
        this.airportSuggestions[search] = result['airports'].sort();
      });
    }
  }
